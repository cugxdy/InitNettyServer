...
/**
 * A {@link ChannelOption}聽allows to configure a {@link ChannelConfig} in a type-safe
...
public class ChannelOption<T> extends UniqueName {
    ...
    // 客户端连接超时时间，NETTY采用的是自定义连接超时定时器检测和超时控制
    ...
    // 用于决定如果网络上仍然有数据向旧的ServerSocket传输数据，是否允许新的ServerSocket绑定到与旧的ServerSocket同样的端口上。
    ...
    // 激活或使用TCP_NODELAY套接字选项，它决定是否使用Nagle算法。当为时延敏感型的应用，建议关闭Nagle算法

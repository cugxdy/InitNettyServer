...
public final class ChannelOutboundBuffer {
     ...
     */
    // 添加指定的message给ChannelOutboundBuffer
    public void addMessage(Object msg, int size, ChannelPromise promise) {
        ...
        if (unflushedEntry == null) {
        	// 当unflushedEntry为null时,设置unflushedEntry字段
    ...
    private void decrementPendingOutboundBytes(long size, boolean invokeLater, boolean notifyWritability) {
        ...
        // 更新totalPendingSize字段值,注意是 (   -size    ):递减操作
        long newWriteBufferSize = TOTAL_PENDING_SIZE_UPDATER.addAndGet(this, -size);
        // 当小于代写最小字节数时,触发fireChannelWritabilityChanged事件
    ...
    public void progress(long amount) {
        ...
        // 当为ChannelProgressivePromise类型时
    ...
    public void removeBytes(long writtenBytes) {
        for (;;) {
            ...
            // 当可读字节数小于已写字节数时
            // 这种情况指的是当有多个ByteBuf时,依次遍历ByteBuf数组,将写完的ByteBuf数组元素删除
    ...
    public ByteBuffer[] nioBuffers() {
        ...
        Entry entry = flushedEntry;
        // 判断entry是否为flushedEntry并判断是否为ByteBuf类型
            ...
            if (!entry.cancelled) {
                ...
                if (readableBytes > 0) {
                    ...
                    if (count == 1) {
                        ...
                        // 将多个ByteBuffer元素添加至NIO_BUFFERS中
    ...
    private static int fillBufferArray(ByteBuffer[] nioBufs, ByteBuffer[] nioBuffers, int nioBufferCount) {
        // 遍历nioBufs数组并将数组元素设置至NIO_BUFFERS中去
        ...
        for (;;) {
            ...
            if (UNWRITABLE_UPDATER.compareAndSet(this, oldValue, newValue)) {
                if (oldValue != 0 && newValue == 0) {
                	// 当为0时,触发fireChannelWritabilityChanged事件
    ...
    private void setWritable(boolean invokeLater) {
        for (;;) {
            ...
            if (UNWRITABLE_UPDATER.compareAndSet(this, oldValue, newValue)) {
                if (oldValue != 0 && newValue == 0) {
                	// 触发fireChannelWritabilityChanged事件
    ...
    // 当超过最大读缓存字节数时,触发fireChannelWritabilityChanged事件
    private void setUnwritable(boolean invokeLater) {
        for (;;) {
            ...
            // 原子性更改值
            if (UNWRITABLE_UPDATER.compareAndSet(this, oldValue, newValue)) {
            	// 当oldValue为0时,触发fireChannelWritabilityChanged事件
                if (oldValue == 0 && newValue != 0) {
                    fireChannelWritabilityChanged(invokeLater);
                }
                break;

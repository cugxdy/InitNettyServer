...
public class LengthFieldPrepender extends MessageToByteEncoder<ByteBuf> {
    ...
    // 它负责在待发送的ByteBuf消息头中增加一个长度字段来标识消息的长度。
    // 它简化了用户的编码器开发,使用户不需要额外去设置这个长度字段
    @Override
    protected void encode(ChannelHandlerContext ctx, ByteBuf msg, ByteBuf out) throws Exception {
        int length = msg.readableBytes() + lengthAdjustment;
        //包含消息长度本身,则在原来的基础上在加上lengthFieldLength的长度
        ...
        // 对消息长度自身所占的字节数进行判断,以便采用正确的方法将长度字段写入到ByteBuf中。
        ...
        case 1:
            ...
            // 创建新的ByteBuf并通过writeByte将长度值写入到ByteBuf中
